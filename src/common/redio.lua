...
function RedIO.new(redstoneSideOrPeripheral, stateFrame, config)
  if DEBUG_LOGS then print("[RedIO] Initializing RedIO instance") end

  local self = setmetatable({}, RedIO)

  -- redstone = type(redstoneSideOrPeripheral) == "string"
  --     and peripheral.wrap(redstoneSideOrPeripheral)
  --     or redstoneSideOrPeripheral

  -- assert(redstone, "Invalid redstone peripheral")
  ...
  if DEBUG_LOGS then print("[RedIO] Initialization complete") end
  return self
end

--- Internal: Verify config consistency and duplicates
function RedIO:_selfChecks()
  if DEBUG_LOGS then print("[RedIO] Performing self checks on configuration") end
  local dup = {}
  ...
  if DEBUG_LOGS then print("[RedIO] Self checks passed") end
end

--- Internal: Setup hooks from state to redstone outputs
function RedIO:_setupBindings()
  if DEBUG_LOGS then print("[RedIO] Setting up output bindings") end
  for key, cfg in pairs(self.config) do
    if cfg.mode == "output" then
      self.stateFrame:onStateChange(key, function(_, newValue)
        if DEBUG_LOGS then print("[RedIO] State changed for " .. key .. ": " .. tostring(newValue)) end
...
function RedIO:_writeRedstone(cfg, value)
  if DEBUG_LOGS then print("[RedIO] Writing redstone output for side " .. cfg.side .. ", value: " .. tostring(value)) end
  if cfg.bundled then
    local cur = redstone.getBundledOutput(cfg.side)
    if value then
      redstone.setBundledOutput(cfg.side, colors.combine(cur, cfg.bundled))
    else
      redstone.setBundledOutput(cfg.side, colors.subtract(cur, cfg.bundled))
    end
  else
    redstone.setOutput(cfg.side, value)
  end
end

--- Poll all configured inputs and write changes to state
function RedIO:pollInputs()
  if DEBUG_LOGS then print("[RedIO] Polling inputs") end
  for key, cfg in pairs(self.config) do
    if cfg.mode == "input" then
      local val
      if cfg.bundled then
        val = colors.test(redstone.getBundledInput(cfg.side), cfg.bundled)
      else
        val = redstone.getInput(cfg.side)
      end

      if self.inputCache[key] ~= val then
        if DEBUG_LOGS then print("[RedIO] Input changed for " .. key .. ": " .. tostring(val)) end
...
function RedIO:set(name, value)
  if DEBUG_LOGS then print("[RedIO] set called for " .. name .. " = " .. tostring(value)) end
  local cfg = self.config[name]
  if not cfg or cfg.mode ~= "output" then return end
  self.stateFrame:setState(name, value)
end

--- Public: Toggle logical output
function RedIO:toggle(name)
  if DEBUG_LOGS then print("[RedIO] toggle called for " .. name) end
...
function RedIO:get(name)
  ...
  if DEBUG_LOGS then print("[RedIO] get called for " .. name .. " = " .. tostring(value)) end
  return value
end
