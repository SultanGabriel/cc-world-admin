...
function PlayerDetectorIO.new(stateFrame, playerDetectorPeripheral,
							  chatBoxPeripheral)
...
function PlayerDetectorIO:_selfCheck()
	...
	assert(self.chatBox and self.chatBox.sendMessage,
		'PlayerDetectorIO requires a valid chatBox peripheral')
...
function PlayerDetectorIO:update()
	-- fixme when running the zone check the player's could also be saved yk
...
function PlayerDetectorIO:_runZoneCheck()
	...
	local function isInZone(pos, zone)
		...
		local x1, x2 = math.min(zone.min.x, zone.max.x),
			math.max(zone.min.x, zone.max.x)
		local z1, z2 = math.min(zone.min.z, zone.max.z),
...
function PlayerDetectorIO:_dimChange(player, dimension)
	...
	local rawDim = nil
	local dimName =
	'Pizdaa Masii... (This is an error message, if you see this, then I fucked up the code, or this shit is fucket.)'
